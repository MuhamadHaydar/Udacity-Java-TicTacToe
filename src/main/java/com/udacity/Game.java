...
public class Game {
    ...
    public String checkGameWinner(char[][] grid) {
        ...
        String resultOfGameTowCase = "";

        char symbols [] = {'x','o'};
        String message[] = {"x wins","o wins"};
        String lastResult[] = {"xxx","ooo"};
        ...
        for (int s = 0;s<symbols.length;s++) {
            ...
            for (int j = 0; j < grid.length; j++) {
                for (int i = 0; i < grid.length; i++) {
                    resultOfGameTowCase = resultOfGameTowCase + grid[j][i];
                }
                if (resultOfGameTowCase.equals(lastResult[s])) {
                    return message[s];
                }
                resultOfGameTowCase = "";
            }
            ...
            for (int j = 0; j < grid.length; j++) {
                for (int i = 0; i < grid.length; i++) {
                    resultOfGameTowCase = resultOfGameTowCase + grid[i][j];
                }
                if (resultOfGameTowCase.equals(lastResult[s])) {
                    return message[s];
                }
            ...
            //checking daiognally case 1 for x and o
            if (grid[0][0] == symbols[s] && grid[1][1] == symbols[s] && grid[2][2] == symbols[s]) {
                return message[s];
            }

            //checking daiognally case 2 for x and o
            if (grid[2][0] == symbols[s] && grid[1][1] == symbols[s] && grid[0][2] == symbols[s]) {
                return message[s];
            }
